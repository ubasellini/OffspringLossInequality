  ...
  for(coh in cohs){
    ...
    for(j in 1:n_col){
      ...
      for(i in 1:n_row){
        ...
        # Fixed on 20220321: I had forgottetn to substract
        # 1 to acount for fact that rows start at 1 and ages at 0
        # This is the old, deprecated, code:
        # (j - 1) < max(c(i - max(reprod_ages), 0)) |
        # (j - 1) > (i - min(reprod_ages))
...
get_unwpp_cohort_rates <- function(lookup_c, reestimate = F, expand_lt_to_future_stable, up_to_year = NA, cohorts_unwpp, cohort_lt, country_key, export = F){
  ...
  if(reestimate){
    ...
    # data <-
    #   lx_un %>%
    #   filter(LocID == names(lookup_c)[1] & Year == 2098)
    ...
    # data <- 
    #   lx_1_1 %>% 
    #   filter(LocID == names(lookup_c)[1] & Year == 1950)
...
ungroup_fert2 <- function(df, add_country_year = F) {
  tryCatch(
    expr = {
      ...
      # out$id <- paste0(unique(df$LocID), "_", unique(df$Year))
      if(add_country_year){
...
get_kin_trees_unwpp <- function(
  ...
  if(!parallel){
    ...
    # con <- unique(asfr_1_1$LocID)[1]; cohort <- 1950
...
convert_period_lt_to_cohort_lt <- 
  function(lt_per, years = 1765:2100, ages = 1:100) {
    ...
    # LTP_df <- LTP_df[LTP_df$Cohort <= year_limit_max, ]
    LTP_df$Year <- NULL
...
reformat_kin_trees <- function(files, dataset = "kins", relatives = c("d", "m")){
  ...
  # c("kins", "kins_by_age_ego", "kins_by_age_kin", "kins_mean_age", 
  # "kins_total")
...
# maol_ugo_by_age <- function(age_child_death, H, surv_mat, what){
#   # browser()
# target_ages <- reprod_ages + age_child_death
# target_ages <- target_ages[target_ages %in% ages_mother]
# 
# ## compute time series
# maol <- numeric(n_coh)
# 
...
#   whi.coh <- which(cohorts==my.coh)
#   surv_vec <- surv_mat[,whi.coh]
#   h <- H[,1+age_child_death,whi.coh]
#   surv_vec <- surv_vec[ages_mother%in%target_ages]
#   h <- h[ages_mother%in%target_ages]
#   maol[i] <- MAOL_ugo_1y(surv_vec = surv_vec, h=h, target_ages = target_ages, what = what)
# }
# maol
# }

# MAOL_ugo_1y <- function(surv_vec,h,target_ages, what = "median"){
#   ## compute nu and scale to 1
#   nu <- surv_vec*h
#   nus <- nu/sum(nu)
#   ## cumulative sum
#   nu_cum <- cumsum(nus)
...
# Plot count curves by child's age at death --------
# # FUNCTIONS to get curve of child loss to plot composition in terms of age at child death
get_cd_by_age_batch <- function(iso){
  ...
  f <- paste0("R/",iso,"_arrays_clean.rdata")
  load(f)
...
share_offspring_died <- function(my.coh, H, Q, POP, asfr, cohorts, n_col, n_row, reprod_ages, ages_mother){
  # share_offspring_died <- function(my.coh, H, Q, POP){
  # browser()
  whi.coh <- which(cohorts==my.coh)
  myH <- H[,1:n_col,whi.coh]
  mypop <- POP[,whi.coh]
